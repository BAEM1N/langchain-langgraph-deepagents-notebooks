...
Interpreters는 experimental API다. `langchain-quickjs>=0.1.0`과 Python 3.11+가 필요하다.
...
`CodeInterpreterMiddleware`는 interpreter state를 turn 사이에 snapshot으로 저장하고 다음 turn에서 복원할 수 있다. 기본값은 `snapshot_between_turns=True`다.
...
3. **snapshot은 interpreter 메모리만 복원한다.** 외부 도구 호출로 발생한 부작용은 되돌리지 않는다.
4. **장기 데이터 분석은 sandbox와 구분한다.** 패키지 설치·파일 처리·셸 실행이 필요하면 sandbox backend를 쓴다.
