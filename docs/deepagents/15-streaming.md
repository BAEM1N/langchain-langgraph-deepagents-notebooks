...
- **`subgraphs=True` + `version="v2"`** — 서브에이전트 내부 이벤트까지 표면화하는 표준 경로
- **`stream_mode`** — `updates` / `messages` / `custom`(또는 리스트)로 어떤 종류의 이벤트를 받을지 결정
- **네임스페이스(`ns` 튜플)** — 이벤트가 메인 에이전트에서 왔는지, 어떤 서브에이전트 내부에서 왔는지 분리하는 라우팅 키

v2 통합 포맷(`type` / `ns` / `data`)은 청크 구조를 단일화해 프런트엔드 라우팅을 단순화한다.
...
- 앱/UI 코드는 **`agent.stream(..., version="v2")`**를 사용한다. v1의 중첩 튜플 포맷은 분기가 복잡하므로 v2 통합 포맷을 권장한다.
