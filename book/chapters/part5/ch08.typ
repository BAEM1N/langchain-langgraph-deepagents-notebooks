...
보이스 에이전트의 핵심은 정답 자체보다 _"얼마나 빨리 첫 오디오를 돌려줄 수 있는가?"_ 입니다. 이 장은 그 지연 시간을 어떻게 분해하고 줄일지에 집중합니다.
]

#chapter-key-points((
  [Sandwich 아키텍처는 STT, Agent, TTS를 겹쳐 실행해 체감 지연을 줄입니다.],
  [성능 목표는 전체 완료 시간이 아니라 첫 출력까지의 시간(TTFT, TTFA)입니다.],
  [스트리밍을 끊는 병목은 종종 모델보다 파이프라인 연결 방식에서 발생합니다.],
))
