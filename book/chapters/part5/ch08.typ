...
이 아키텍처는 _Sandwich 패턴_ (STT -\> Agent -\> TTS)으로, 각 계층이 스트리밍으로 연결되어 sub-700ms 레이턴시를 목표로 합니다.
...
- _Agent_: `astream()`으로 토큰 단위로 응답을 스트리밍 — 전체 응답 생성 완료를 기다리지 않음
...
스트리밍 없이 동기적으로 처리하면, 각 단계가 완전히 끝나야 다음 단계가 시작됩니다. 예를 들어 에이전트가 200 토큰 응답을 생성하는 데 3초가 걸린다면, TTS는 3초 후에야 시작됩니다. 스트리밍에서는 에이전트의 _첫 토큰이 나오는 즉시_ TTS가 음성 합성을 시작하므로, 사용자는 에이전트가 아직 응답을 생성하는 동안에도 이미 음성을 듣기 시작합니다.

=== 멀티모달 콘텐츠 블록과 `output_version="v1"`

LangChain 1.x 의 메시지는 텍스트, 이미지, 오디오, 비디오 등 다양한 modality 를 표현할 수 있는 _content blocks_ 구조를 사용합니다. `ChatOpenAI(output_version="v1")` 로 모델을 생성하면, 응답이 표준 블록 리스트 (`[{"type": "text", "text": "..."}, {"type": "audio", "audio": {...}}]`) 형태로 직렬화됩니다.
...
#table(
  ...
  [이미지 입력·출력],
  [`audio`],
  [오디오 입력 (STT 전 단계) 또는 출력 (TTS 직전 단계)],
  [`tool_use` / `tool_result`],
  [도구 호출과 결과],
)

Sandwich 아키텍처에서는 STT 가 텍스트를 만들어 에이전트에 넘기지만, _멀티모달 모델(gpt-5.4 audio, claude-sonnet-4-6 등)_ 을 직접 사용할 때는 오디오 블록을 메시지 콘텐츠로 직접 주입할 수 있습니다. `output_version="v1"` 은 이 직렬화 형식을 안정화합니다.
...
LangChain의 `RunnableGenerator`를 사용하면 비동기 제너레이터를 _Runnable 파이프라인_에 통합할 수 있습니다. STT 출력 -\> Agent 처리 -\> TTS 입력이라는 전체 데이터 흐름을 하나의 파이프라인으로 구성할 수 있습니다.
...
WebSocket으로 클라이언트와 양방향 오디오 스트리밍을 처리합니다. `asyncio.gather`로 수신과 송신을 _동시 실행_합니다.
...
음성 에이전트의 도구는 _빠른 응답_이 중요합니다. 도구 실행 시간이 길어지면 전체 파이프라인의 레이턴시가 커지므로:
