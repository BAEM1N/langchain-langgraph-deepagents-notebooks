...
- *써야 하는 경우*: 다단계 검색, 대용량 도구 출력, 전문 역할 분리, 메인 에이전트의 컨텍스트 보호가 중요할 때
- *쓰지 않는 경우*: 단순 단일 단계 작업, 중간 추론을 메인 컨텍스트가 계속 참조해야 하는 경우, 위임 오버헤드가 이득보다 큰 경우
- 핵심 판단 기준은 _"결과만 돌려받아도 되는가, 아니면 중간 과정까지 메인 에이전트가 계속 봐야 하는가?"_ 입니다.]
