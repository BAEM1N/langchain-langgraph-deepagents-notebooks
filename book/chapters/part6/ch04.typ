...
프롬프트는 사실상 _코드_지만, 엔지니어가 아닌 사람이 자주 고치고 주기도 짧습니다. 매번 재배포하지 않으려면 프롬프트를 _애플리케이션 코드와 분리된 저장소_에 두고 버전 핀을 박아야 합니다. LangSmith Prompt Hub가 그 역할을 합니다. push/pull API, commit SHA vs tag, 템플릿 엔진 선택, Playground 실험, CI 핀 전략을 다룹니다.
...
프로덕션 배포 슬롯은 `:prod` 태그로 받고, _회귀 테스트는 commit SHA를 핀_해야 합니다. 그래야 "통과한 테스트 이후 누군가 프롬프트를 고쳤다"는 사고를 자동으로 막습니다.
...
3장의 experiment에 `prompt_commit`을 metadata로 붙이면 _어떤 커밋 기준에서 낸 수치인지_ UI에서 바로 추적됩니다.
