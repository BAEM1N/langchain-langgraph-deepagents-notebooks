...
최초 로그인 시 역할 선택 → 모드 선택 → 홈 → 퀵스타트 다이얼로그 순서로 4단계 온보딩을 거칩니다. Technical 역할과 LangSmith(code-first) 모드를 선택해 개발자용 흐름으로 진입합니다.
...
LangChain을 거치지 않는 전처리/후처리 유틸리티 함수도 `@traceable`로 트레이스에 편입합니다. 부모 트레이스 아래 자식 span으로 묶이므로 "왜 이 도시로 쿼리를 보냈는가" 같은 추적이 됩니다.
...
UI 프로젝트 페이지 우상단의 _Analytics_ 탭에서 프로젝트 단위 총 비용/토큰 시계열을 봅니다. 모델별 단가는 LangSmith가 자동으로 계산해 `total_cost` 필드에 채우고, 자체 호스팅 모델이라면 _Settings → Model pricing_ 에서 단가를 직접 지정할 수 있습니다.
